...
public class EGLBase {
	...
	public static class EglSurface {
		...
		EglSurface(final EGLBase egl, final Object surface) {
	        ...
	        if (DEBUG) Log.i(TAG, String.format("EglSurface:size(%d,%d)", mWidth, mHeight));
		}
	...
	public EglSurface createOffscreen(final int width, final int height) {
		if (DEBUG) Log.i(TAG, "createOffscreen:");
	...
	private void init(EGLContext shared_context, final boolean with_depth_buffer, final boolean isRecordable) {
        ...
        if (DEBUG) Log.i(TAG, "EGLContext created, client version " + values[0]);
...
//		if (DEBUG) Log.i(TAG, "makeCurrent:");
        if (mEglDisplay == null) {
            if (DEBUG) Log.i(TAG, "makeCurrent:eglDisplay not initialized");
        }
	...
	private void makeDefault() {
		if (DEBUG) Log.i(TAG, "makeDefault:");
...
//		if (DEBUG) Log.i(TAG, "createContext:");
    ...
    private void destroyContext() {
		if (DEBUG) Log.i(TAG, "destroyContext:");
    ...
    private EGLSurface createWindowSurface(final Object nativeWindow) {
		if (DEBUG) Log.i(TAG, "createWindowSurface:nativeWindow=" + nativeWindow);
    ...
    private EGLSurface createOffscreenSurface(final int width, final int height) {
		if (DEBUG) Log.i(TAG, "createOffscreenSurface:");
        final int[] surfaceAttribs = {
	...
	private void destroyWindowSurface(EGLSurface surface) {
		if (DEBUG) Log.i(TAG, "destroySurface:");
        ...
        if (DEBUG) Log.i(TAG, "destroySurface:finished");
	}
