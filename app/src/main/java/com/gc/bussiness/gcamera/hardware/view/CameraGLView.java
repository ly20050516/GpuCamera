package com.gc.bussiness.gcamera.hardware.view;
/*
...
//********************************************************************************
	...
	private static final class CameraSurfaceRenderer
		...
		public CameraSurfaceRenderer(final CameraGLView parent) {
			if (DEBUG) Log.i(TAG, "CameraSurfaceRenderer:");
...
//			if (DEBUG) Log.i(TAG, "onSurfaceCreated:Gl extensions: " + extensions);
		...
		private final void updateViewport() {
			...
			if (parent != null) {
				...
				case SCALE_KEEP_ASPECT_VIEWPORT:
				{
					...
					if (DEBUG) Log.i(TAG, String.format("xy(%d,%d),size(%d,%d)", x, y, width, height));
				...
				case SCALE_CROP_CENTER:
				{
					...
					Log.i(TAG, String.format("size(%1.0f,%1.0f),scale(%f,%f),mat(%f,%f)",
		...
		private int flip = 0;
        private final int VIDEO_FPS = 60 / 10; // ~10fps,60 express camera preview fps, 10 express we expected.
		/**
		...
		public void onDrawFrame(final GL10 unused) {
            ...
            // draw to video encoder
            // TODO: 2017/10/22 We will controller fps here
            if (flip++ % VIDEO_FPS == 0) {
                if(flip == VIDEO_FPS) {
                    flip = 0;
                }
