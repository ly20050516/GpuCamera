...
public class CaptureButton extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.i("LiuTag", "ACTION_DOWN state = " + state);
                if (event.getPointerCount() > 1 || state != STATE_IDLE) {
                    break;
                }
                event_Y = event.getY();     //记录Y值
                state = STATE_PRESS;        //修改当前状态为点击按下
                Log.i("LiuTag", "state = " + state);
                ...
                if ((button_state == BUTTON_STATE_ONLY_RECORDER || button_state == BUTTON_STATE_BOTH)) {
                    ...
                    Log.i("LiuTag", "button_state = " + button_state);

                }
                break;
            case MotionEvent.ACTION_MOVE:
                Log.i("LiuTag", "ACTION_MOVE state = " + state);
            ...
            case MotionEvent.ACTION_UP:
                Log.i("LiuTag", "ACTION_UP state = " + state);
                //根据当前按钮的状态进行相应的处理
    ...
    private class LongPressRunnable implements Runnable {
        ...
        public void run() {
            ...
            Log.d("LiuTag", "LongPressRunnable run: STATE_LONG_PRESS");
